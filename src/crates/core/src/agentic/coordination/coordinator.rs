...
fn ensure_supported_agent_type(agent_type: &str) -> BitFunResult<()> {
    if agent_type.trim().eq_ignore_ascii_case("claw") {
        return Err(BitFunError::Validation(
            "Claw sessions are disabled".to_string(),
        ));
    }
...
impl ConversationCoordinator {
    ...
    pub async fn create_session_with_workspace_and_creator(
        ...
        ensure_supported_agent_type(&agent_type)?;
        let session = self
            .session_manager
    ...
    pub async fn ensure_assistant_bootstrap(
        ...
        Ok(AssistantBootstrapEnsureOutcome::Skipped {
            session_id,
    ...
    pub async fn restore_session(
        ...
        let session = self
            ...
            .await?;
        ensure_supported_agent_type(&session.agent_type)?;
        Ok(session)
    }
    ...
    pub async fn list_sessions(&self, workspace_path: &Path) -> BitFunResult<Vec<SessionSummary>> {
        Ok(self
            ...
            .await?
            .into_iter()
            .filter(|summary| !summary.agent_type.trim().eq_ignore_ascii_case("claw"))
            .collect())
    }
